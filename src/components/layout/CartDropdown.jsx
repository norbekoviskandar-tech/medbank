                ...
                <>
                  <div className="space-y-6 max-h-[300px] overflow-y-auto pr-2 scroll-smooth">
                    {cart.map((item) => (
                      <div key={item.id} className="flex items-center gap-4 group relative p-3 rounded-2xl hover:bg-slate-50 transition-all">
                        <div className="w-16 h-16 rounded-2xl bg-rose-50 flex items-center justify-center text-rose-500 shrink-0 border border-rose-100/50">
                          <Activity size={24} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <h4 className="font-black text-slate-800 text-sm truncate">{item.title}</h4>
                          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">90-Day Access</p>
                          <div className="mt-1">
                            <span className="text-[#1d46af] font-black text-sm">$199.00</span>
                          </div>
                        </div>
                        <button 
                  ...
                  <div className="mt-10 pt-8 border-t border-slate-50">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <span className="text-2xl font-black text-slate-900 leading-none">$199.00</span>
                    </div>
                    <button 
                      onClick={() => {
                        setIsCartOpen(false);
