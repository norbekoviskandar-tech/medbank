...
export default function ManageQuestionsPage() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      "id", "system", "subject", "topic", "stem", "choices", "correct",
      "explanationCorrect", "explanationWrong", "summary",
      "references", "cognitiveLevel", "published"
    ];

    const rows = dataToExport.map(q => headers.map(h => {
      let val = q[h];

      // Special handling for objects/arrays (choices)
          ...
          imported = lines.slice(1).map((line, lineIdx) => {
            ...
            headers.forEach((h, i) => {
              ...
              if (h === 'published') {
                ...
                if (val) {
                  try {
                    // Handle both standard JSON and single-quote escaped JSON if any
                    const jsonStr = val.startsWith('"') && val.endsWith('"')
                      ? val.slice(1, -1).replace(/""/g, '"')
                      : val;
                    val = JSON.parse(jsonStr);
                  } catch (e) {
                    console.warn(`Line ${lineIdx + 2}: Failed to parse choices. Expected JSON array. Value:`, val);
                    val = [];
                  }
                } else {
                  val = [];
