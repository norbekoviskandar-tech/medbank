...
import { BookOpen, User, Mail, Lock, ArrowRight, AlertCircle, CheckCircle2 } from "lucide-react";
...
export default function SignupPage() {
  ...
  async function handleSignup(e) {
    ...
    try {
      ...
      setError(err?.message || err || "Failed to create account. Please try again.");
      setIsLoading(false);
    }
  }
        ...
        <div className="glass rounded-[2.5rem] p-8 shadow-2xl shadow-primary/5 sm:p-10">
          {isSuccess ? (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-500/10">
                <CheckCircle2 size={48} />
              </div>
              <h2 className="text-2xl font-bold">Account Created!</h2>
              <p className="mt-2 text-zinc-600 dark:text-zinc-400">Redirecting to login...</p>
            </div>
          ) : (
          ...
          {!isSuccess && (
            <div className="mt-8 text-center text-sm">
              ...
              <a href="/login" className="font-bold text-primary hover:underline">Sign in now</a>
            </div>
          )}
        </div>
      </div>
