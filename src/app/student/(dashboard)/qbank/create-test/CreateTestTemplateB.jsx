...
export default function CreateTestTemplateB({ questions, userId, productConfig }) {
  ...
  const subjectCounts = useMemo(() => {
    ...
    // Sequential dependency: Pool must be chosen first
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className={`border-b border-zinc-800/50 ${activeFilters.length === 0 ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
        <div className="flex items-center justify-between py-3 cursor-pointer group" onClick={() => activeFilters.length > 0 && setIsSystemsOpen(!isSystemsOpen)}>
      ...
      {/* SUBJECTS */}
      <div className={`border-b border-zinc-800/50 ${activeFilters.length === 0 ? 'opacity-40 grayscale pointer-events-none' : ''}`}>
        <div className="flex items-center justify-between py-3 cursor-pointer group" onClick={() => activeFilters.length > 0 && setIsSubjectsOpen(!isSubjectsOpen)}>
          <div className="flex items-center gap-2">
            <div onClick={(e) => {
              e.stopPropagation();
